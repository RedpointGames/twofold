...
export interface ServerTelemetry {
  ...
  /**
   * Called when there is a recoverable error during page rendering on the server; the error will propagate to SSR. This should return the digest to be associated with the error.
   *
   * You should almost always return defaultTelemetryBehaviour() if you implement this hook, as the digest is needed for SSR and client-side code to directly detect 'not found' and 'unauthorized' errors.
   */
  ...
  /**
   * Called when the server receives an error from SSR rendering that results in SSR failing to render any response at all. SSR also calls 'onClientSideRenderError' to mirror the behaviour that would be seen in the browser.
   *
   * This hook can be used to replace the page content depending on the error.
   */
  onServerSideReceivedSsrError?: TelemetryHook<
    (
      context: ServerErrorContext,
    ) => Promise<TelemetryDefaultable<ReplacementResponse>>
  >;
